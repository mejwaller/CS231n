...
def SoftmaxLoss(X,y,W,reg=1):
    ...
    loss+=SVMDataLoss.regL2norm(W,reg)
    
    return loss, p

def SoftmaxLossBias(X,y,W,b,reg=1):
    """
    X = data - e.g. 500 x 3072 (500 images, 3072 pixel values per image)
    y = 500 - correct class for each image
    W = 3072 x 10 - weight matrix. 10 values for each class for each pixel
    b = bias vector e.g. 1 X 3072
    """
    scores = X.dot(W) + b#500x10 - scores for each image for each class
    scores-=np.max(scores,axis=1)[:, np.newaxis]#subtract max value for the scores for each image
