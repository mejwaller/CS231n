...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        if self.use_batchnorm:
            ...
            gamma = gamma1
            bn_param = bn_param1
            conv_layer, cache_conv_layer = lu.conv_norm_relu_pool_forward(
                x, w, b, conv_param, pool_param, gamma, beta, bn_param)
        else:
            conv_layer, cache_conv_layer = lu.conv_relu_pool_forward(
        ...
        if self.use_batchnorm:
            ...
            beta = beta2
            bn_param = bn_param2
            hidden_layer, cache_hidden_layer = lu.affine_norm_relu_forward(
                x, w, b, gamma, beta, bn_param)
        else:
            hidden_layer, cache_hidden_layer = lu.affine_relu_forward(x, w, b)
        ...
        w = W3
        b = b3
        scores, cache_scores = lu.affine_forward(x, w, b)
        ...
        dx3, dW3, db3 = lu.affine_backward(dscores, cache_scores)
        dW3 += self.reg * W3

        # Backprop into first layer
        if self.use_batchnorm:
            dx2, dW2, db2, dgamma2, dbeta2 = lu.affine_norm_relu_backward(
                dx3, cache_hidden_layer)
        else:
            dx2, dW2, db2 = lu.affine_relu_backward(dx3, cache_hidden_layer)
        ...
        if self.use_batchnorm:
            dx, dW1, db1, dgamma1, dbeta1 = lu.conv_norm_relu_pool_backward(
                dx2, cache_conv_layer)
        else:
            dx, dW1, db1 = lu.conv_relu_pool_backward(dx2, cache_conv_layer)
